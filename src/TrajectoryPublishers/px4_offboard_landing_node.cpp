...
class LandingTrajectoryPublisherNode : public rclcpp::Node {
public:
  LandingTrajectoryPublisherNode() : Node("px4_offboard_trajectory_publisher") {
    ...
    odometry_sub_ = this->create_subscription<px4_msgs::msg::VehicleOdometry>
        ("fmu/out/vehicle_odometry", qos, std::bind(&LandingTrajectoryPublisherNode::odometryCallback, this, std::placeholders::_1));
    // Subscriber to groundtruth for diagnostics
    groundtruth_sub_ = this->create_subscription<px4_msgs::msg::VehicleLocalPosition>
        ("/fmu/out/vehicle_local_position_groundtruth_v1", qos, std::bind(&LandingTrajectoryPublisherNode::groundtruthCallback, this, std::placeholders::_1));
    ...
    // Timer: 100 Hz (0.01 s)
    timer_ = this->create_wall_timer(0.01s, std::bind(&LandingTrajectoryPublisherNode::publishLandingTrajectory, this));
    offboardTimer_ = this->create_wall_timer(0.33s, [=]() {publishOffboardControlModeMsg();});
...
private:
  ...
  // Camera frame ID (hardcoded)
  // const std::string camera_frame_id_ = "t2_cruza_vtol_0/camera_link/imager";
  const std::string camera_frame_id_ = "x500_mono_cam_0/camera_link/imager";
  const std::string platform_frame_id_ = "platform";
  ...
  // r_cam_b_b: position of camera in body frame, expressed in body frame
  const Eigen::Vector3d r_cam_b_b_{0.0, 0.0, -0.1}; //x500
  // const Eigen::Vector3d r_cam_b_b_{0.0, 0.0, -0.03}; //t2
  ...
  // Eigen::Vector3d drone_position_W_prev_;
  ...
  rclcpp::TimerBase::SharedPtr offboardTimer_;
  ...
  // (Adapted from controller_node.h for coordinate frame transformations)
  ...
  // void predictFusedPosition() {
  //   // Prediction step: integrate odometry velocity at 100 Hz
  //   // Continuously estimates position between tag updates
  //   // estimated_position_W_ += drone_velocity_W_ * dt_;
  //   estimated_position_W_ += drone_position_W_ - drone_position_W_prev_;
  // }
  ...
  void publishLandingTrajectory() {
    // Monitor actual callback frequency
    // monitorFrequency();
    ...
    // Update fused position estimate: 100 Hz prediction from odometry
    // predictFusedPosition();
    ...
    if (phase_ != Phase::PHASE_3_LANDING) {
      publishTrajectoryPoint();
      // Publish PX4 trajectory setpoint
      publishTrajectorySetpoint();
    }
  ...
  void updatePhase1() {
    // Phase 1: Move to (0, 0, 5) with proportional velocity control
    // Use actual drone position from odometry for real-time feedback
    
    // Try to get platform position from TF for monitoring (used for transition check and logging)
    updateTagPosition();
    ...
    // r_velocity_W_ = drone_velocity_W_; 
    // // Saturate r_velocity_W_ jump to max_acceleration_xy_ * dt_
    // Eigen::Vector3d velocity_change = desired_velocity - drone_velocity_W_;
  ...
  void updatePhase2() {
    // Phase 2: Use TF to get tag position and move to (0, 0, 0) with controlled descent
    // if (!updateTagPosition()) {
    //   RCLCPP_WARN(this->get_logger(), "Failed to get tag transform, staying in Phase 2");
    //   return;
    // }
    ...
    // Eigen::Vector3d position_error = phase_2_target_ - drone_position_W_;
    ...
    // Z component: force constant descent rate (negative = downward in ENU)
    // desired_velocity(2) = -phase2_descent_rate_z_;
    ...
    // r_position_W_ = drone_position_W_ + r_velocity_W_ * dt_;
    r_position_W_ = r_position_W_ + r_velocity_W_ * dt_;
    // r_position_W_ = estimated_position_W_ + r_velocity_W_ * dt_;
    // r_position_W_ = position_W_ + r_velocity_W_ * dt_;
  ...
  bool updateTagPosition() {
    try {
      ...
      auto transform = tf_buffer_->lookupTransform(
          ...
          // tf2::durationFromSec(0.001)   // Lookup timeout 1ms
      );
      ...
      // Eigen::Vector3d r_plat_b_plat = R_plat_cam_ * r_plat_b_cam;
      ...
      // Log position periodically (every second at 100Hz = every 100 iterations)
      // static int log_counter = 0;
      // if (++log_counter >= 100) {
      //   RCLCPP_INFO(this->get_logger(), "[Fusion] Tag: [%.3f, %.3f, %.3f] | Estimate: [%.3f, %.3f, %.3f]", 
      //              position_W_(0), position_W_(1), position_W_(2),
      //              estimated_position_W_(0), estimated_position_W_(1), estimated_position_W_(2));
      //   RCLCPP_INFO(this->get_logger(), "[Odometry] Drone position: [%.3f, %.3f, %.3f]", 
      //              drone_position_W_(0), drone_position_W_(1), drone_position_W_(2));
      //   log_counter = 0;
      // }
  ...
  void publishTrajectoryPoint() {
    ...
    // Debug log: print setpoints
    // static int log_counter = 0;
    // if (++log_counter >= 50) {  // Log every 0.5 seconds at 100Hz
    //   RCLCPP_INFO(this->get_logger(), 
    //               "Phase %d | Pos: [%.3f, %.3f, %.3f] | Vel: [%.3f, %.3f, %.3f] | Acc: [%.3f, %.3f, %.3f]",
    //               (int)phase_,
    //               r_position_W_(0), r_position_W_(1), r_position_W_(2),
    //               r_velocity_W_(0), r_velocity_W_(1), r_velocity_W_(2),
    //               r_acceleration_W_(0), r_acceleration_W_(1), r_acceleration_W_(2));
    //   log_counter = 0;
    // }
  ...
  void publishTrajectorySetpoint() {
    ...
    // Convert reference vectors from ENU to NED
    ...
    // Position setpoint (in NED frame)
    ...
    // Acceleration feedforward (in NED frame)
  ...
  void odometryCallback(const px4_msgs::msg::VehicleOdometry::SharedPtr odom_msg) {
    ...
    // drone_position_W_prev_ = drone_position_W_;
  ...
  void groundtruthCallback(const px4_msgs::msg::VehicleLocalPosition::SharedPtr gt_msg) {
    ...
    // Assuming groundtruth is in NED frame like odometry
